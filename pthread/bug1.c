...
void *inc_count(void *idp) 
{
  ...
  for (i=0; i < TCOUNT; i++) {
  	if (i >= 8)
		printf("i = %ld, thread %ld prepare to lock mutex\n", i, my_id);
    ...
    */
	...
	printf("end lock %ld, i = %d\n", my_id, i);
	fflush(stdout);
    /* Do some work so threads can alternate on mutex lock */
    sleep(1);
	printf("next step, thread %ld\n", my_id);

    }
	printf("pre to exit thread %ld\n", my_id);
	
  pthread_exit( (void*)my_id );
	
	printf("thread %ld exit...\n", my_id); // Dead code...HAHA~ 
...
int main(int argc, char *argv[])
{
 ...
 void* exit_info;
  ...
  for (i = 0; i < NUM_THREADS; i++) {
    rc = pthread_join(threads[i], &exit_info);
	if (rc == 0)
	{
		printf("Main: completed join with thread %d having a singal of %ld\n", i, (long)exit_info);
	}
  }
